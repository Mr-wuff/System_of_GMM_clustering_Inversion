...
### 1.You have selected the parameters that suit you best
If you are an individual researcher or your hardware configuration for development is not particularly high, then I suggest you set the tolerance to 0.001, the expected convergence value to 1, and the clustering level to 5 to achieve large-scale data computations, such as for CYGNSS, which covers a global-scale grid of data.
### 2.You possess substantial financial resources or a strong research team
However, if you have sufficient confidence in your hardware configuration for development, then I suggest setting the tolerance to below 0.0001. Of course, as the tolerance decreases, the number of iterations should be appropriately increased. Then, when the tolerance is set to 0.0001, I recommend the number of iterations to be between 500 and 1000, the clustering level to be above 10, and the expected convergence value to be below 0.5. Of course, the expected convergence value should not be set too low, although the program has a setting that will not consider the expected convergence value when reaching the iteration limit and obtain the result. But this also means that you cannot achieve the goal of minimizing the deviation by lowering the expected convergence value, so an appropriate setting is the best choice.

### 3.You have chosen to challenge yourself
Oh, by the way, let me add something. If you are confident about your hardware configuration and have set some extreme parameter values, there is a high probability that you will see the progress bar of the code almost remain unchanged. Don't worry, this does not mean that there is a problem with your data or that the code has malfunctioned. The code is likely running normally (unless there is a fault notification or a fault pop-up in the log area on the right), but it may take a considerable amount of time for you to obtain the results ^m^
